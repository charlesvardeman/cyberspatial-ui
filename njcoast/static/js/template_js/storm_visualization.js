...
function unfollowClick(parent) {
    ...
    unfollow_storm_template.append(storm_name_text).append("<span title=\"Follow\"><i id=\"" + storm_name + "\" class=\"fa fa-plus-circle\" aria-hidden=\"true\" " + "onclick=\"followClick($(this))\"></i></span>").append("<p class=\"follow-unfollow\">not following</p>");
...
function check_scenario_map_layers(storm_name) {
    ...
    if (layers_checked.length == 0) {
        ...
        if (layers_checked.indexOf("windfield_box scenario_box") > -1 && layers_checked.indexOf("surge_box scenario_box") > -1) {
            ...
            scenario_controls.find(".analysistype").attr("disabled", false);
            $('#' + storm_name + '_update_btn').attr("disabled", false);
        } else if (layers_checked.indexOf("windfield_box scenario_box") > -1 && layers_checked.indexOf("runup_box scenario_box") > -1 || layers_checked.indexOf("surge_box scenario_box") > -1 && layers_checked.indexOf("runup_box scenario_box") > -1) {
            scenario_controls.find(".sealevel").attr("disabled", false);
            scenario_controls.find(".coastal_protection").attr("disabled", false);
            scenario_controls.find(".tides").attr("disabled", false);
            scenario_controls.find(".analysistype").attr("disabled", false);
        ...
        if (layers_checked.indexOf("windfield_box scenario_box") > -1 && layers_checked.indexOf("surge_box scenario_box") > -1 && layers_checked.indexOf("runup_box scenario_box") > -1) {
            scenario_controls.find(".sealevel").attr("disabled", false);
            scenario_controls.find(".coastal_protection").attr("disabled", false);
            scenario_controls.find(".tides").attr("disabled", false);
            scenario_controls.find(".analysistype").attr("disabled", false);
...
function updateMapClick(storm_name) {
    ...
    if (surge_checkbox.is(":checked")) {
        ...
        //load heatmap data
        heatmap_url = s3_path + "surge/heatmap__slr_" + parseInt(sea_level * 10) + "__tide_" + tides + "__analysis_" + analysis_type + ".json";
