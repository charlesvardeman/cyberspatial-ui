...
class SignUpForm(ProfileCreationForm):
    ...
    county = forms.ModelChoiceField(queryset=NJCCounty.objects, required=False, help_text='Required if "County" is selected for Region Level.')
    municipality = forms.ModelChoiceField(queryset=NJCMunicipality.objects, required=False, help_text='Required if "Municipality" is selected for Region Level.')
